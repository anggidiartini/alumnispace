                        ...
                        <tr class="data-row {{ $isEventSoldOut ? 'event-row-sold-out' : '' }}" @if($isEventSoldOut) style="background-color: rgba(254, 242, 242, 0.55);" @endif>
                                  ...
                                  @if(in_array($col, ['avatar', 'thumbnail', 'photo_path', 'cover_photo', 'company_logo']))
                                    ...
                                    @elseif($col === 'study_status')
                                        ...
                                        <select class="status-dropdown" data-id="{{ ($table_key === 'alumni_boards') ? $row->alumni_profile_id : $row->id }}" data-table="alumni_profiles" data-column="study_status" style="padding: 6px 10px; border-radius: 6px; border: 1px solid #d0e1f0; background: {{ $statusVal === 'Aktif' ? '#ecfdf5' : '#fef2f2' }}; color: {{ $statusVal === 'Aktif' ? '#047857' : '#b91c1c' }}; font-weight: 600; outline: none; cursor: pointer;">
                                            ...
                                            <option value="Non-aktif" {{ $statusVal !== 'Aktif' ? 'selected' : '' }}>● Tidak Aktif</option>
                                        </select>
                            ...
                            <td style="text-align: center;">
                                <div class="action-badge">
                                    <a href="{{ route('admin.galleries.show', $row->id) }}" class="btn-action btn-detail">
                                        <i class="fa-solid fa-eye"></i> Detail
                                    </a>
                                    <a href="{{ route('admin.galleries.edit', $row->id) }}" class="btn-action btn-edit">
                                        <i class="fa-solid fa-pen-to-square"></i> Edit
                                    </a>
                                    <form class="delete-form" action="{{ route('admin.galleries.destroy', $row->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
