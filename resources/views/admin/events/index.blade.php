...
<div class="crud-card-full">
    ...
    <table id="eventsDataTable" class="data-table display nowrap" style="width:100%">
        <thead>
            <tr>
                <th class="col-number-header">NO</th>
                <th>
                    <div class="dt-th-box">
                        <span class="dt-th-title">Agenda Acara</span>
                ...
                <th>
                    <div class="dt-th-box">
                        <span class="dt-th-title">Kuota Peserta</span>
                        <span class="dt-sort-arrow"></span>
                    </div>
                </th>
                <th>
        ...
        <tbody>
            @forelse($events as $event)
                @php
                    $evUsed = \Illuminate\Support\Facades\DB::table('event_registrations')
                        ->where('event_id', $event->id)
                        ->where('status', '!=', 'cancelled')
                        ->sum('quantity');
                    $evTotal = (int) ($event->quota ?? 0);
                ...
                <tr class="data-row {{ $isSoldOut ? 'event-row-sold-out' : '' }}" @if($isSoldOut) style="background-color: rgba(254, 242, 242, 0.55);" @endif>
                    ...
                    <td>
                        ...
                        @else
                            <span style="color: #64748b; font-size: 12px;">Tidak dibatasi</span>
                        @endif
                    </td>
                    <td>
                        @php
                    ...
                    <td style="text-align: center;">
                        <div class="action-badge">
                            <a href="{{ route('admin.events.show', $event->id) }}" class="btn-action btn-detail">
                                <i class="fa-solid fa-eye"></i> Detail
                            </a>
                            <a href="{{ route('admin.events.edit', $event->id) }}" class="btn-action btn-edit">
                                <i class="fa-solid fa-pen-to-square"></i> Edit
                            </a>
                            <form class="delete-form" action="{{ route('admin.events.destroy', $event->id) }}" method="POST" style="display:inline-flex;">
                                @csrf
                                @method('DELETE')
...
<div class="delete-modal-backdrop" id="deleteModalBackdrop" aria-hidden="true" hidden>
    <div class="delete-modal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle">
        ...
        <h3 id="deleteModalTitle">Anda yakin ingin menghapus acara ini?</h3>
...
<script>
    $(document).ready(function () {
        ...
        // ========================================================
        const tableElement = $('#eventsDataTable');
