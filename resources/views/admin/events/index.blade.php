...
<div style="background:#fff;border:1px solid var(--border-color);border-radius:12px;padding:24px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;gap:12px;">
        <div><h2 style="color:var(--text-main);">Daftar Acara & Agenda</h2><p style="color:var(--text-muted);font-size:13px;">Kelola data pada tabel events.</p></div>
        <a href="{{ route('admin.events.create') }}" style="padding:10px 16px;border-radius:8px;background:var(--color-primary);color:#fff;text-decoration:none;">Tambah Acara</a>
    </div>
    <div style="overflow-x:auto;"><table class="data-table" style="width:100%;border-collapse:collapse;font-size:13px;">
        <thead><tr style="background:var(--bg-main);text-align:left;"><th style="padding:12px;">Nama Acara</th><th style="padding:12px;">Tanggal</th><th style="padding:12px;">Lokasi</th><th style="padding:12px;">Status</th><th style="padding:12px;">Aksi</th></tr></thead>
        <tbody>
        @forelse($events as $event)
            <tr style="border-bottom:1px solid var(--border-color);"><td style="padding:12px;">{{ $event->title }}</td><td style="padding:12px;">{{ $event->event_date?->format('d M Y') }}</td><td style="padding:12px;">{{ $event->venue ?? ucfirst($event->location_type) }}</td><td style="padding:12px;">{{ ucfirst($event->status) }}</td><td style="padding:12px;white-space:nowrap;text-align:center;"><div class="action-badge"><a href="{{ route('admin.events.show', $event->id) }}" class="btn-action btn-detail"><i class="fa-solid fa-eye"></i> Detail</a><a href="{{ route('admin.events.edit', $event->id) }}" class="btn-action btn-edit"><i class="fa-solid fa-pen-to-square"></i> Edit</a><form action="{{ route('admin.events.destroy', $event->id) }}" method="POST" style="display:inline;margin-left:10px;" onsubmit="return confirm('Hapus acara ini?');">@csrf @method('DELETE')<button type="button" class="btn-action btn-delete delete-trigger"><i class="fa-solid fa-trash"></i> Hapus</button></form></td></tr>
        @empty
            <tr><td colspan="5" style="padding:24px;text-align:center;color:var(--text-muted);">Belum ada acara.</td></tr>
        @endforelse
        </tbody>
    </table>
