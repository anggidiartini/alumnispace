...
<div style="background:#fff;border:1px solid var(--border-color);border-radius:12px;padding:24px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;gap:12px;"><div><h2 style="color:var(--text-main);">Daftar Album Foto</h2><p style="color:var(--text-muted);font-size:13px;">Kelola data pada tabel albums.</p></div><a href="{{ route('admin.albums.create') }}" style="padding:10px 16px;border-radius:8px;background:var(--color-primary);color:#fff;text-decoration:none;">Tambah Album</a></div>
    <div style="overflow-x:auto;"><table class="data-table" style="width:100%;border-collapse:collapse;font-size:13px;"><thead><tr style="background:var(--bg-main);text-align:left;"><th style="padding:12px;">Nama Album</th><th style="padding:12px;">Kategori</th><th style="padding:12px;">Target Angkatan</th><th style="padding:12px;">Foto</th><th style="padding:12px;">Aksi</th></tr></thead><tbody>
        @forelse($albums as $album)<tr style="border-bottom:1px solid var(--border-color);"><td style="padding:12px;">{{ $album->title }}</td><td style="padding:12px;">{{ $album->category }}</td><td style="padding:12px;">{{ $album->target_generation ?? '-' }}</td><td style="padding:12px;">{{ $album->photos_count }}</td><td style="padding:12px;white-space:nowrap;text-align:center;"><div class="action-badge"><a href="{{ route('admin.albums.show', $album->id) }}" class="btn-action btn-detail"><i class="fa-solid fa-eye"></i> Detail</a><a href="{{ route('admin.albums.edit', $album->id) }}" class="btn-action btn-edit"><i class="fa-solid fa-pen-to-square"></i> Edit</a><form action="{{ route('admin.albums.destroy', $album->id) }}" method="POST" style="display:inline;margin-left:10px;" onsubmit="return confirm('Hapus album ini?');">@csrf @method('DELETE')<button type="button" class="btn-action btn-delete delete-trigger"><i class="fa-solid fa-trash"></i> Hapus</button></form></div></td></tr>@empty
        <tr><td colspan="5" style="padding:24px;text-align:center;color:var(--text-muted);">Belum ada album.</td></tr>@endforelse
    </tbody></table>
