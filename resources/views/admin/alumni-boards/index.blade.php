...
<div class="crud-card-full">
    <div class="table-header">
        ...
        <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
            ...
            <a href="{{ route('admin.alumni-boards.create') }}" class="btn-add">
    ...
    <table id="alumniBoardsDataTable" class="data-table display nowrap" style="width:100%">
        <thead>
            <tr>
                ...
                <th>
                    <div class="dt-th-box">
                        <span class="dt-th-title">Nama Alumni</span>
                        <span class="dt-sort-arrow"></span>
                    </div>
                </th>
                <th>
                    <div class="dt-th-box">
                        <span class="dt-th-title">Jabatan</span>
                        <span class="dt-sort-arrow"></span>
                    </div>
                </th>
                <th>
                    <div class="dt-th-box">
                        <span class="dt-th-title">Periode Kepengurusan</span>
                        <span class="dt-sort-arrow"></span>
                    </div>
                </th>
                <th>
        ...
        <tbody>
            @forelse($boards as $board)
                <tr class="data-row">
                    ...
                    <td style="font-weight: 500; text-align: left; padding-left: 18px;">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            @if(!empty($board->avatar))
                                <img src="{{ asset($board->avatar) }}" style="width: 34px; height: 34px; border-radius: 50%; object-fit: cover; border: 1px solid #e2e8f0;">
                            @else
                                <div style="width: 34px; height: 34px; border-radius: 50%; background: #e0f2fe; color: #0369a1; display: grid; place-items: center; font-weight: 700; font-size: 13px;">
                                    {{ strtoupper(substr($board->alumni_name ?? 'A', 0, 1)) }}
                                </div>
                            @endif
                            <span>{{ $board->alumni_name }}</span>
                        </div>
                    </td>
                    <td style="font-weight: 600; color: #0a4174;">{{ $board->position }}</td>
                    <td>
                        <span style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 4px 10px; border-radius: 6px; font-weight: 600; font-size: 12px; color: #334155;">
                            {{ $board->period_name }}
                        </span>
                    </td>
                    <td>
                    ...
                    <td style="text-align: center;">
                        <div class="action-badge">
                            <a href="{{ route('admin.alumni-boards.show', $board->id) }}" class="btn-action btn-detail">
                                <i class="fa-solid fa-eye"></i> Detail
                            </a>
                            <a href="{{ route('admin.alumni-boards.edit', $board->id) }}" class="btn-action btn-edit">
                                <i class="fa-solid fa-pen-to-square"></i> Edit
                            </a>
                            <form class="delete-form" action="{{ route('admin.alumni-boards.destroy', $board->id) }}" method="POST" style="display:inline-flex;">
                                @csrf
                                @method('DELETE')
...
<div class="delete-modal-backdrop" id="deleteModalBackdrop" aria-hidden="true" hidden>
    <div class="delete-modal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle">
        ...
        <h3 id="deleteModalTitle">Anda yakin ingin menghapus data pengurus ini?</h3>
...
<script>
    $(document).ready(function () {
        ...
        // ========================================================
        const tableElement = $('#alumniBoardsDataTable');
