...
@section('content')
    ...
    <div class="crud-card-full">
        <div class="table-header">
            ...
            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                <a href="{{ route('admin.categories.index', ['type' => 'articles']) }}" class="btn btn-outline-secondary">
                    <i class="fa-solid fa-boxes-stacked me-1"></i> Kategori
                </a>
        ...
        <table id="articlesDataTable" class="data-table display nowrap" style="width:100%">
            <thead>
                <tr>
                    ...
                    <th>
                        <div class="dt-th-box">
                            <span class="dt-th-title">Tanggal Terbit</span>
                            <span class="dt-sort-arrow"></span>
                        </div>
                    </th>
                    ...
                    <tr class="data-row">
                        ...
                        <td>
                            @if (!empty($article->thumbnail))
                                <img src="{{ asset($article->thumbnail) }}" class="preview-img-mini" alt="Thumbnail"
                                    style="width: 50px; height: 50px; border-radius: 6px; object-fit: cover; border: 1px solid var(--border-color, #e2e8f0);"
                                    onerror="this.onerror=null; this.src='data:image/svg+xml;utf8,<svg xmlns=\'http://w3.org\' width=\'40\' height=\'40\' viewBox=\'0 0 24 24\' fill=\'none\' stroke=\'%2394a3b8\' stroke-width=\'1.5\'><rect width=\'18\' height=\'18\' x=\'3\' y=\'3\' rx=\'2\'/><circle cx=\'9\' cy=\'9\' r=\'2\'/><path d=\'m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21\'/></svg>';">
                            @else
                                <span style="color: var(--text-muted, #64748b); font-style: italic; font-size: 11px;">Tidak
                                    ada foto</span>
                            @endif
                        </td>
                        ...
                        <td>
                            <select class="status-dropdown" data-id="{{ $article->id }}" data-table="articles"
                                data-column="is_published"
                                style="padding: 6px 10px; border-radius: 6px; border: 1px solid #d0e1f0; background: {{ $article->is_published ? '#ecfdf5' : '#fef2f2' }}; color: {{ $article->is_published ? '#047857' : '#b91c1c' }}; font-weight: 600; outline: none; cursor: pointer;">
                                <option value="1" {{ $article->is_published ? 'selected' : '' }}>● Diterbitkan
                                </option>
                        ...
                        <td style="text-align: center;">
                            <div class="action-badge">
                                <a href="{{ route('admin.articles.show', $article->id) }}" class="btn-action btn-detail">
                                    <i class="fa-solid fa-eye"></i> Detail
                                </a>
                                <a href="{{ route('admin.articles.edit', $article->id) }}" class="btn-action btn-edit">
    ...
    <div class="delete-modal-backdrop" id="deleteModalBackdrop" aria-hidden="true" hidden>
        <div class="delete-modal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle">
            ...
            <h3 id="deleteModalTitle">Anda yakin ingin menghapus artikel ini?</h3>
        ...
        $(document).ready(function() {
            ...
            document.querySelectorAll('.status-dropdown').forEach(dropdown => {
                dropdown.addEventListener('change', function(e) {
                    ...
                    const originalValue = isNumericToggle ? (newValue == '1' ? '0' : '1') : (
                        newValue === 'Aktif' ? 'Tidak Aktif' : 'Aktif');
