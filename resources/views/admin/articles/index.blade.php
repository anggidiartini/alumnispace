...
<div style="background:#fff;border:1px solid var(--border-color);border-radius:12px;padding:24px;">
    @if(session('success'))
        <div style="padding:12px 16px;margin-bottom:20px;border:1px solid #22c55e;border-radius:8px;background:#ecfdf5;color:#166534;">{{ session('success') }}</div>
    @endif

    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;gap:12px;">
        <div>
            <h2 style="color:var(--text-main);">Daftar Artikel & Berita</h2>
            <p style="color:var(--text-muted);font-size:13px;">Kelola seluruh artikel pada tabel articles.</p>
        </div>
        <a href="{{ route('admin.articles.create') }}" style="padding:10px 16px;border-radius:8px;background:var(--color-primary);color:#fff;text-decoration:none;">Tambah Artikel</a>
    </div>

    <div style="overflow-x:auto;">
        <table class="data-table" style="width:100%;border-collapse:collapse;font-size:13px;">
            <thead><tr style="background:var(--bg-main);text-align:left;"><th style="padding:12px;">Judul</th><th style="padding:12px;">Kategori</th><th style="padding:12px;">Status</th><th style="padding:12px;">Aksi</th></tr></thead>
            <tbody>
            @forelse($articles as $article)
                <tr style="border-bottom:1px solid var(--border-color);">
                    <td style="padding:12px;">{{ $article->title }}</td>
                    <td style="padding:12px;">{{ $article->category }}</td>
                    <td style="padding:12px;">{{ $article->is_published ? 'Diterbitkan' : 'Draf' }}</td>
                    <td style="padding:12px;white-space:nowrap;text-align:center;"><div class="action-badge">
                        <a href="{{ route('admin.articles.show', $article->id) }}" class="btn-action btn-detail"><i class="fa-solid fa-eye"></i> Detail</a>
                        <a href="{{ route('admin.articles.edit', $article->id) }}" class="btn-action btn-edit"><i class="fa-solid fa-pen-to-square"></i> Edit</a>
                        <form action="{{ route('admin.articles.destroy', $article->id) }}" method="POST" style="display:inline;margin-left:10px;" onsubmit="return confirm('Hapus artikel ini?');">
                            @csrf @method('DELETE')
                            <button type="button" class="btn-action btn-delete delete-trigger"><i class="fa-solid fa-trash"></i> Hapus</button>
                        </form>
                    </td>
                </tr>
            @empty
                <tr><td colspan="4" style="padding:24px;text-align:center;color:var(--text-muted);">Belum ada artikel.</td></tr>
            @endforelse
            </tbody>
        </table>
