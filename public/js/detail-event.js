...
(function () {
    ...
    function updateQuotaDisplay(registeredCount, quota) {
        const quotaCountEl = document.querySelector(
            "[data-role='quota-count']",
        );
        ...
        if (quotaCountEl)
            quotaCountEl.textContent = registeredCount + " / " + quota;
        if (infoQuotaEl)
            infoQuotaEl.textContent =
                registeredCount + " dari " + quota + " peserta";
        if (remainingEl) {
            ...
            fetch(config.registerUrl, {
                ...
                .then(function (result) {
                    ...
                    if (result.ok && result.data && result.data.success) {
                        ...
                        if (
                            typeof result.data.registered_count !==
                                "undefined" &&
                            config.quota
                        ) {
                            updateQuotaDisplay(
